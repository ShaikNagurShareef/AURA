"""
AURA - Medical PDF Report Generator

Produces a hospital-grade PDF with:
  - Letterhead with AURA branding
...
# ?? Colour Palette ????????????????????????????????????????????????????????????
BLUE_DARK  = (0, 70, 127)    # AURA primary navy
...
class MedicalReportPDF(FPDF):
    """Hospital-grade FPDF layout for AURA diagnostic reports."""
    ...
    def header(self):
        ...
        self.cell(0, 8, "AURA  .  AI Clinical Reporting System", align="C",
    ...
    def footer(self):
        ...
        self.cell(
            0, 5,
            "AURA AI Report -- Not a substitute for professional clinical evaluation  |  "
            f"Page {self.page_no()}",
            align="C",
        )
...
def generate_diagnostic_pdf(
    ...
    pdf.cell(
        0, 5,
        "?  PRISM -- AI Diagnostic Analysis Engine  |  AURA Healthcare Platform",
        align="C", new_x="LMARGIN", new_y="NEXT",
    )
    pdf.set_font("Helvetica", "I", 8)
    pdf.set_text_color(*GREY_TEXT)
